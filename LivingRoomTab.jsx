...
const ROOM_BACKGROUND = "https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/68e72a9f947d6831aa76e519/08e6e44f3_2.png";

export default function LivingRoomTab({ user, onUpdate }) {
  ...
  const handleReturnToAttic = async (itemId) => {
    try {
      ...
      console.error("Error returning item to attic:", error);
    }
  };

  if (loading) {
    return <div className="text-center py-8">Loading Living Room...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <Card className="border-2 border-[#1165b3]/20">
        ...
        <CardContent>
          <div 
            className="relative w-full aspect-square bg-cover bg-center rounded-2xl overflow-hidden bg-gradient-to-br from-amber-100 to-orange-50"
            ...
            {items.length === 0 && (
              <div className="absolute inset-0 flex items-center justify-center">
                <p className="text-gray-500 text-center px-4">
                  No items in living room yet.<br />Visit the Nook or move items from other rooms!
                </p>
              </div>
            )}
          </div>
      ...
      <Card className="bg-white rounded-3xl shadow-lg border-2 border-[#66bfad]/30">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-[#1165b3]">🏠 Living Room Items</CardTitle>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <p className="text-gray-500 text-center py-8">
              No items in living room yet. Visit the Nook or move items from other rooms!
            </p>
          ) : (
